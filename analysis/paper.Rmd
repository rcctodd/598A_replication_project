...
```{r load packages and set working dir, include=TRUE}
#adding this line as a git test
#install specific versions of pakages called out in the paper
require(devtools)

#install_version("geomorph", version = "3.0.6", repos = "http://cran.us.r-project.org")
library(geomorph) # geomorph 3.0.6

#install_version("shapes", version = "1.2.3", repos = "http://cran.us.r-project.org")
...
```{r plotting digitized specs, include=TRUE}
                          #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
...
```{r check size vs shape, include=TRUE}
                          #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-
...
```{r test mean shape diff, include=TRUE}
                          #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-
...
```{r bilateral symmetry, include=TRUE}
...
```{r replicatio target, include=TRUE}

#Fig.8.Correlation matrix of Havor lance dimensions showing Pearson's r(lowerpanel), accompanying bivariate plots (upperpanel) and  distributions (histograms with kernel density estimation, on the diagonal). Red=Ejsbøl, green=Nydam, blue=Illerup. 

# Figure is 7 by 7; variables are the following, along with column ref in lances_M
#total length 5
#blade length 6
#socket length 7
#socket (max) 8?
...
names(lances_M)
#pearson's R
#historam with kernal estimation
#bivarate plots
...
ggpairs(lances_M, columns = c("total_length", "blade_length", "socket_length", "sock_bdiam", "sock_sdiam", "width", "thickness"), 
        upper=list(continuous = "points", mapping = aes(color = site)),
        lower=list(continuous = "cor"),
        ggsave("Images/replication_fig_8v1.png"))

#try constructing min and max vars

lances_M$socket_max = pmax(lances_M$sock_bdiam,lances_M$sock_sdiam, na.rm = TRUE)
lances_M$socket_min = pmin(lances_M$sock_bdiam,lances_M$sock_sdiam, na.rm = TRUE)


ggpairs(lances_M, columns = c("total_length", "blade_length", "socket_length", "socket_max", "socket_min", "width", "thickness"), 
        upper=list(continuous = "points", mapping = aes(color = site)),
        lower=list(continuous = "cor"),
        ggsave("Images/replication_fig_8v2.png"))

#notes for those who follow:
# check variable names - dont think sock_bdiam etc. is the same as min max width
# histogram as well as density on middle plots
# trend line as well as scatter on upper plots
# color match
# label match
```
